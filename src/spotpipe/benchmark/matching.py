"""Ground-truth <-> prediction matching for the benchmark (build stage 4).

Pairs predicted spots to ground-truth spots **by location**, one-to-one, within
a maximum-distance gate (a radius in pixels). This is the first step of every
metric: a matched pair contributes to true-positive detection and to the
per-spot intensity/ratio residuals; an unmatched GT spot is a false negative; an
unmatched prediction is a false positive.
...
external baseline are matched identically. Two assignment strategies are offered:

* ``greedy`` (default): sort all within-gate candidate pairs by distance and
  assign nearest-first, skipping any spot already taken. Deterministic and has no
...
Both are one-to-one. For the harness's sanity check -- matching a ground-truth
table against *itself* -- both yield a perfect, zero-distance pairing (every spot
matches itself), which is exactly the validation the benchmark relies on.
"""
