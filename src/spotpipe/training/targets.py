...
This module is the boundary where dense / incorrect supervision could creep in,
so the rules are explicit and narrow (see CLAUDE.md and Prompt 3):
...
* **Heatmap** = a tight Gaussian blob rendered at the integer centre pixel
  ...
  Rendering at the integer centre guarantees a clean ``==1`` positive for the
  CenterNet focal loss; the sub-pixel part is recovered entirely by the offset
  head. The blob ``sigma`` is a config parameter and is kept tight so close
  spots in the high-overlap regime do not merge into one peak.
...
regardless; the offset / intensity targets are written **last-spot-wins** (the
later row in the table overwrites the earlier). This is documented and accepted
because exact centre collisions at integer resolution are vanishingly rare.
